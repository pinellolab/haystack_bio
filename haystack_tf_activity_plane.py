...
print '\n[H A Y S T A C K   TFs  A C T I V I T Y   P L A N E]'
...
if not os.path.exists(haystack_motifs_output_folder):
    err("The haystack_motifs_output_folder specified: %s doesn't exist!")
    sys.exit(1)
...
df_gene_mapping=pd.read_table(gene_expression_samples_filename,header=None,index_col=0,names=['Sample_name','Sample_file'])
    ...
    if current_motif_id in motif_mapping.index:
        ...
        for gene_name in set(map(str.upper,motif_mapping.ix[current_motif_id].GENES.split(','))):
            ...
            try:
                ...
                warn('The expression values of the gene %s  is not present. Skipping it.' % gene_name.upper())
                continue
