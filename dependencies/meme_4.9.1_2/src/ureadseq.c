...
struct ReadSeqVars {
  short err;
  long choice, nseq, topnseq; /* changed from short to long - James J (UQ) */
...
Local void readPhylipInterleaved(struct ReadSeqVars *V)
{
  ...
  si= V->s;
  skipwhitespace(si);
  V->topnseq= atol(si); /* changed from atoi to atol - James J (UQ) */
    ...
    if (feof(V->f)) V->err = eNoData;
      ...
      case kPAUP    : {
        ...
        while (!done) {
          ...
          if (NULL != (cp=strstr( V->s, "ntax=")) )  V->topnseq= atol(cp+5); /* changed from atoi to atol - James J (UQ) */
...
char *readSeqFp(
      const long whichEntry_,  /* index to sequence in file, changed from short to long - James J (UQ) */
      FILE  *fp_,   /* pointer to open seq file */
      const long  skiplines_,
      const short format_,      /* sequence file format */
      long  *seqlen_,     /* return seq size */
      long *nseq_,       /* number of seqs in file, for listSeqs(), changed from short to long - James J (UQ) */
