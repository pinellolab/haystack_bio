...
MHMM_T* allocate_mhmm(ALPH_T alph, int num_states) {
  ...
  an_mhmm->alph = alph;
  an_mhmm->alph_size = alph_size(alph, ALPH_SIZE);
  an_mhmm->ambigs = alph_size(alph, AMBIG_SIZE);
  an_mhmm->alphabet = strdup(alph_string(alph));

  // Allocate the background distribution.
  an_mhmm->background = allocate_array(alph_size(alph, ALL_SIZE));
   ...
   MHMM_T*        new_mhmm)
{
  ...
  new_mhmm->alph = an_mhmm->alph;
  copy_string(&(new_mhmm->alphabet), an_mhmm->alphabet);
  new_mhmm->background = allocate_array(alph_size(an_mhmm->alph, ALL_SIZE));
   ...
   MHMM_STATE_T * a_state)   /* State to be analyzed. */
{
  ...
  i_max = 0;
  for (i_freq = 0; i_freq < alph_size(alph, ALPH_SIZE); i_freq++) {
    if (log_space) {
