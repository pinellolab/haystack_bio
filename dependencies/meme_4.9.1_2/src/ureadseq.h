...
/* changed nseq from short to long - James J (UQ) */
extern char *listSeqs(const char *filename, const long skiplines,
                       const short format, long *nseq, short *error );

/* changed whichEntry and nseq from short to long - James J (UQ) */
extern char *readSeq(const long whichEntry, const char *filename,
                      const long skiplines, const short format,
                      long *seqlen, long *nseq, short *error, char *seqid );

/* changed whichEntry_ and nseq_ from short to long - James J (UQ) */
extern char *readSeqFp(const long whichEntry_, FILE  *fp_,
...
/* changed whichEntry and nseq from short to long - James J (UQ) */
extern char *readASNSeq(const long whichEntry, const char *filename,
                const long skiplines, const short format,
                long *seqlen, long *nseq, short *error, char **seqid );
#endif
