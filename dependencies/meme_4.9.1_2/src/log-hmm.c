...
static void convert_to_from_log_state(
  ...
  // Compute odds for the emission distribution in the source state.
    ...
    for (i_emit = 0; i_emit < alph_size(alph, ALPH_SIZE); i_emit++) {
  ...
  if (to_log) {
    target_state->emit = allocate_array(alph_size(alph, ALL_SIZE));
    target_state->emit_odds = allocate_array(alph_size(alph, ALL_SIZE));
  ...
  if (LOG_DEBUG) {
    for (i_emit = 0; i_emit < alph_size(alph, ALL_SIZE); i_emit++) {
...
void convert_to_from_log_hmm (
  ...
  myfree(target_hmm->alphabet);
  target_hmm->alphabet = strdup(source_hmm->alphabet);
