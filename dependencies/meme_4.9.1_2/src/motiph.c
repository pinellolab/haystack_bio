...
int main(int argc, char *argv[]) {
  ...
  if (alph != DNA_ALPH) die("Expected DNA motifs");
  ...
  if (scan_both_strands == TRUE ) {
    ...
    normalize_subarray(0, alph_size(DNA_ALPH, ALPH_SIZE), 0.0, bg_freqs);
  ...
  if (column_freqs_type == EMPIRICAL_COL_FREQS) {
    ...
    alignment_col_freqs_list = get_alignment_column_freqs_list(DNA_ALPH, filenames, TRUE);
