...
void print_cisml_multi_pattern_match(
  ...
  fprintf(
    out,
    "<mem:match cluster-id=\"%s\" seq-name=\"%s\" start=\"%d\" stop=\"%d\" " 
...
**********************************************************************/
CISML_T* read_cisml(
	const char* 		cisml_filename // name of the cisml file
) {
...
void read_patterns_from_cisml(
  ...
  *num_patterns = (xpath_obj->nodesetval ? xpath_obj->nodesetval->nodeNr : 0);
  ...
  for (i_pattern = 0; i_pattern < *num_patterns; i_pattern++) {
    ...
    // Mark pattern as complete, since no more matches can be added.
    pattern->is_complete = TRUE;

	  // add pattern to cisml
...
void read_scanned_sequences_from_cisml(
  ...
  for (cur_node = first_sequence; cur_node; cur_node = cur_node->next) {
    ...
    //	  cur_node->name,
    //	  read_xml_node_property(cur_node, "accession")
    //	);
...
/**********************************************************************
  ...
  Reads matches in a scanned_sequence from a cisml file
**********************************************************************/
void   read_matched_elements_from_cisml(
  ...
  // Read all matches from the scanned sequence
  xmlNode *cur_node = NULL;
  for (cur_node = firstmatch; cur_node; cur_node = cur_node->next) {
    ...
    // Add match to pattern
    add_pattern_matched_element_no_heap(pattern, element);
...
void print_cisml(FILE* out, CISML_T *cisml, BOOLEAN_T print_header,
  ...
  print_cisml_start(out, program_name, print_header, stylesheet, print_namespace);
