 ...
 * 
 * $Log$
 * Revision 1.1  2005/07/29 17:18:51  nadya
 * Initial revision
 *
 */
...
static inline void test_value (int test) { 
  if (!test) {
    fprintf(stderr, "FATAL: IO function failed\n"); exit(1); 
  }
}
...
int chooseFormat( boolean quietly)
{
  ...
  int   midi, i, outform;
    ...
    if (quietly)
      ...
      test_value(fgets(sform, 128, stdin) != NULL);
...
long      whichlist[kMaxwhichlist+1]; /* changed short to long - James J */
...
int readopt( char *sopt)
{
  ...
  long    n=0; /* changed from short to long - James J (UQ) */
  ...
  if (*sopt == '?') {
    ...
    *sparam= '\0';
    ...
    if (checkopt( false, sopt, "-item", 2)) {
      ...
      do {
        while (*cp!=0 && !isdigit((int) *cp)) cp++;
        if (*cp!=0) {
          n = atol( cp); /* changed from atoi to atol - James J (UQ) */
    ...
    if (checkopt( false, sopt, "-format", 5)) {/* -format=phylip, -f2, -form=phylip */
      if (*sparam==0) { for (sparam= sopt+2; isalpha((int) *sparam); sparam++) ; }
    ...
    if (checkopt( false, sopt, "-output", 3)) {/* -output=myseq */
      if (*sparam==0) { for (sparam= sopt+3; isalpha((int) *sparam); sparam++) ; }
      strcpy( oname, sparam);
      foo = fopen( oname, "w");
    ...
    if (checkopt( false, sopt, "-width", 2)) {
      if (*sparam==0) { for (sparam= sopt+2; !isdigit((int) *sparam) && *sparam!=0; sparam++) ; }
      n= atoi( sparam);
  ...
  *oname = 0;
  ...
  if (dopipe && !gotinputfile) {
	...
	*/
    ...
    while ((c = getc(stdin)) != EOF) test_value(write(fd, &c, 1) == 1);
    close(fd);
    gotinputfile= true;
    }
  ...
  if (askout && !closeout && !quietly) {
    ...
    test_value((oname= fgets(onamestore, 128, stdin)) != NULL);
  ...
  do {
    ...
    while (!gotinputfile) {
      ...
      test_value((stemp= fgets(stempstore, 256, stdin)) != NULL);
      ...
      while (whichSeq > 0 && whichSeq <= nseq) {
        ...
        if (manyout) {
          if ( whichSeq == 1 ) erralert(eOneFormat);
            ...
            test_value(freopen( stemp, "w", fout) != NULL);
