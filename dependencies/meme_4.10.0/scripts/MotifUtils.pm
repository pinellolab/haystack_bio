...
require Exporter;
our @ISA = qw(Exporter);
our @EXPORT_OK = qw(seq_to_intern matrix_to_intern intern_to_iupac intern_to_meme intern_to_pspm read_background_file meme_header motif_id motif_width parse_double round motif_corr);
...
sub seq_to_intern {
  ...
  if (defined($also)) {
    my $any_bits = ($bg->{dna} ? $dna_bits{N} : $protein_bits{X});
    for (my $i = 0; $i < length($also); $i++) {
      $nums{substr($also, $i, 1)} = $any_bits;
    }
  }
...
#   );
#
#   Returns an array listing the correlation for each pair of 
#   motifs. The correlation of the ith motif with the jth motif is at position
#   i * nmotifs + j.
#
#   In scalar context returns the correlation between the first 2 motifs.
#
#   Assumes all motifs have the same alphabet.
#
sub motif_corr {
  my (@motifs) = @_;
  return () unless (@motifs);
  my @letters = ($motifs[0]->{bg}->{dna} ? @dna_alphabet : @protein_alphabet);
  my $alpha_mean = 1 / scalar(@letters);
  my @PCC = ();
  for (my $m1 = 0; $m1 < scalar(@motifs); $m1++) {
    for (my $m2 = 0; $m2 < $m1; $m2++) {
      my $best_score = -1;
      for (my $order = 0; $order < 2; $order++) {
        my $motif1 = $motifs[($order ? $m1 : $m2)];
        my $motif2 = $motifs[($order ? $m2 : $m1)];
        my $m1w = $motif1->{width};
        my $m2w = $motif2->{width};
        my $shortw = ($m1w < $m2w ? $m1w : $m2w);
        for (my $alignment = 0; $alignment < $m1w; $alignment++) {
          my $sum = 0;
          for (my $a1 = 0, my $a2 = $alignment; $a1 < $m1w && $a2 < $m2w; $a1++, $a2++) {
            my $top = 0;
            my $low1 = 0;
            my $low2 = 0;
            for (my $l = 0; $l < scalar(@letters); $l++) {
              my $letter = $letters[$l];
              my $x = $motif1->{pspm}->{$letter}->[$a1] - $alpha_mean;
              my $y = $motif2->{pspm}->{$letter}->[$a2] - $alpha_mean;
              $top += $x * $y;
              $low1 += $x * $x;
              $low2 += $y * $y;
            }
            $sum += $top / sqrt($low1 * $low2);
          }
          my $score = $sum / $shortw;
          $best_score = $score if ($score > $best_score);
        }
      }
      $PCC[$m1 * scalar(@motifs) + $m2] = $best_score;
      $PCC[$m2 * scalar(@motifs) + $m1] = $best_score;
    }
  }
  for (my $m = 0; $m < scalar(@motifs); $m++) {
    $PCC[$m * scalar(@motifs) + $m] = 1;
  }
  if (wantarray) {
    return @PCC;
  } else { # return the correleation between the first 2 motifs
    return $PCC[1];
  }
}
  ...
  for (my $i = 0; $i < scalar(@chars); $i++) {
    ...
    if (!$inside_bracket) { #not inside a bracket expression
      if ($char eq '[') { #found the start of a bracket expression
        ...
        $bracket_value = 0;
        $bracket_start = $i;
      } elsif ($char =~ m/^([\Q$alpha\E])$/i) { #found a normal character
