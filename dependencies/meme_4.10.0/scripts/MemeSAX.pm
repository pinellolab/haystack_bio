...
my $ST_IN_MODEL = [
  ...
  {ELE => 'mask_type', RPT => $RPT_OPT},
  {ELE => 'min_key', RPT => $RPT_OPT},
  {ELE => 'max_key', RPT => $RPT_OPT},
  {ELE => 'negfile', RPT => $RPT_OPT},
  {ELE => 'kmer', RPT => $RPT_OPT},
  {ELE => 'ncopies', RPT => $RPT_OPT},
...
my $ST_IN_TRSET = [ # training set
  ...
  {ELE => 'letter_frequencies', TO => $ST_IN_LFREQS, 
    START => \&_start_ele_letter_frequencies, END => \&_end_ele_letter_frequencies}
];
my $ST_IN_MEME = [
