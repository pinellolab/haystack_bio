 ...
 *   1 if m1->score > m2->score,
 *   0 if m1->score = m2->score,
 *  -1 if m1->score < m2->score,
 *
 ***********************************************************************/
 ...
 *
 ***********************************************************************/
void set_mcast_match_gc(double gc, MCAST_MATCH_T *match) {
  match->gc = gc;
}
 ...
 *
 ***********************************************************************/
void set_mcast_match_start(size_t start, MCAST_MATCH_T *match) {
  match->start = start;
}
 ...
 *
 ***********************************************************************/
void set_mcast_match_stop(size_t stop, MCAST_MATCH_T *match) {
  match->stop = stop;
}
...
void print_mcast_match_as_cisml(
  ...
  fprintf(out, " pvalue=\"%.3g\"", match_pvalue);
  fprintf(out, ">\n");
  MOTIF_HIT_T *motif_hit = retrieve_next_object(mcast_match->motif_hits);
  while(motif_hit != NULL) {
    ...
    fprintf(
      out,
      "<matched-element start=\"%zd\" stop=\"%zd\" pvalue=\"%.3g\">\n",
      hit_start,
      hit_stop,
      hit_pvalue
    );
...
/**********************************************************************
  print_mcast_matches_as_text

  Print a heap of mcast_matches as plain text.
**********************************************************************/
void print_mcast_matches_as_text(
  BOOLEAN_T stats_available,
  int num_matches,
  MCAST_MATCH_T **mcast_matches,
  MHMMSCAN_OPTIONS_T *options
  ...
  for (i = 0; i < num_matches; ++i) {
    ...
    if (!stats_available
      ...
      fprintf(
        stdout,
        "cluster-%d\t%s\t%zd\t%zd\t%g\t%.3g\t%.3g\t%.3g\t%s\n",
        id,
        seq_name,
        start,
        stop,
