...
const char const *pvalue_method_names[] = {"Wilcoxon rank-sum test", "Fisher's exact test", "multihg", 
...
void rsc_getopt(int argc, char *argv[]) {
  const int num_options = 25; // change this if the number of options changes
  ...
  while(simple_getopt(&option_name, &option_value, &option_index) > 0) {
    ...
    if (strcmp(option_name, "control") == 0) {
...
void rsc_scan_sequences() {
  ...
  if (args.control_filename) {
    ...
    jsonwr_lng_prop(args.json_output, "count", num_seqs-args.fix_partition);
    jsonwr_end_object_value(args.json_output);
  }
...
void rsc_get_scores(int argc, char *argv[]) {
  ...
  for (i = 0; i < motifs.num; ++i) {
    ...
    if (RANKSUM_METHOD == args.pvalue_method) {
      ...
      if (corrected_p_val <= args.pvalue_report_threshold) {
        rsc_print_results("%i. Fisher's exact test p-value of motif %s %s top %i seqs: %.4g (Corrected p-value: %.4g)\n",
    ...
    for (i=0; i < seq_num - 1; i++) {
      ...
      if (args.verbose >= HIGH_VERBOSE) {
        fprintf(stderr, "Fisher's exact test p-value of motif %s top %i seqs (tp >= observed): %g\n",
            get_motif_st_id(motif_at(motifs.motifs, motif_index*2)), i+1, pright);
      }
