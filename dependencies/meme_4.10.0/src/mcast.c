...
typedef struct mcast_options {
  ...
  BOOLEAN_T use_synth; // Generate synthetic sequence if needed to calc. E-values.
  BOOLEAN_T text_only; // Don't generate XML/HTML
...
static void process_command_line(
  ...
  const char *usage = 
    "USAGE: mcast [options] <query> <database>\n"
     ...
     "  [ --prior-dist <value> ]      File containing distribution of position specific priors (default none)\n"
     "  [ --alpha <value> ]           The fraction of all TF binding sites that are binding sites\n"
     "                                for the TF of interest. Used in the calcultion of PSP. (default 1.0)\n"
...
static void build_scan_options_from_mcast_options(
  ...
  const char* CONSTANTS_STYLESHEET = "constants.xsl";
  scan_options->constants_stylesheet_path
    = make_path_to_file(etc_dir, CONSTANTS_STYLESHEET);
  scan_options->constants_stylesheet_local_path
    = make_path_to_file(mcast_options->output_dirname, CONSTANTS_STYLESHEET);
  ...
  const char* BED_STYLESHEET = "mcast-to-bed.xsl";
  scan_options->bed_stylesheet_path 
    = make_path_to_file(etc_dir, BED_STYLESHEET);
  scan_options->bed_stylesheet_local_path
    = make_path_to_file(mcast_options->output_dirname, BED_STYLESHEET);
  scan_options->bed_path
    = make_path_to_file(mcast_options->output_dirname, "mcast.bed");
  ...
  scan_options->text_output = mcast_options->text_only;
...
static void mcast_read_and_score(
  ...
  *num_seqs = 0;
  ...
  while (sequence != NULL) {
    ...
    BOOLEAN_T is_complete_seq = is_complete(sequence);
    ...
    while (
      ...
      if (score_set->n == 2 * MIN_SCORES && got_evd == FALSE) {
        evd_set = calc_distr(
      ...
      set_mcast_match_score(viterbi_score + options->dp_thresh, mcast_match);
      set_mcast_match_gc(t_or_gc, mcast_match);
...
static void write_mcast_results(
  ...
  BOOLEAN stats_available = evd_set.n > 0 ? TRUE : FALSE;
  ...
  if (options->text_output == TRUE) {
    print_mcast_matches_as_text(stats_available, num_matches, mcast_matches, options);
    return;
  }
  ...
  copy_file(options->bed_stylesheet_path, options->bed_stylesheet_local_path);
  ...
  copy_file(options->constants_stylesheet_path, options->constants_stylesheet_local_path);
  ...
  print_xml_filename_to_filename_using_stylesheet(
    options->cisml_path,
    options->bed_stylesheet_local_path,
    options->bed_path
  );
}
...
static void mcast(
  ...
  evd_set = calculate_statistics(&scan_options, num_matches, mcast_matches, score_set);
...
int main(int argc, char* argv[]) {
  ...
  MOTIF_T *motifs = NULL;
  MHMM_T *hmm = NULL;
  read_motifs(&options, &background, &num_motifs, &motifs);
