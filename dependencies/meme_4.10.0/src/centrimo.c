...
typedef struct options {
  ...
  BOOLEAN_T disc; // Use Fisher's Exact Test
...
static void output_centrimo_text(FILE* text_file, BOOLEAN_T scan_separately,
   BOOLEAN_T negative_sequences, MOTIF_STATS_T* stats, int motifN, int sequence_length) {
  //vars
  MOTIF_DB_T *db;
  MOTIF_T *motif;
  WIN_STATS_T *window;
  ...
  fprintf(text_file, "%s %*s\t", get_id(motif), pad, alt[0] ? alt : get_motif_id(motif));
...
int main(int argc, char *argv[]) {
  ...
  if (!options.bg_source) {
    bg_freqs = calc_bg_from_file(options.alphabet, options.seq_source, false);
  }
