...
)    
{
  ...
  for (i = 0; i < score_set->n; ++i) {
    ...
    t_or_gc = score_set->scores[i].gc;
    double pvalue = evd_set_pvalue(score, t_or_gc, q, *evd_set);
  ...
  for (i = 0; i < num_matches; ++i) {
    if (matches[i]) {
      ...
      double t_or_gc = get_mcast_match_gc(matches[i]);
      double pvalue = evd_set_pvalue(score, t_or_gc, q, *evd_set);
  ...
  // The matches came sorted by score, but because we're stratifying scores
  // by GC content we may not be in p-value order so we have to explicitly
  // sort by p-values.
  qsort(matches, num_matches, sizeof(MCAST_MATCH_T*), rev_compare_mcast_match_pvalues);
  ...
  // Calculate q-values.
  /* FIXME CEG turn q-values back on when done debugging pvalues! */
  compute_qvalues(
