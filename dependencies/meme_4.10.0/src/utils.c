 ...
 * The buffer must be at least 7 bytes long. The output is null terminated.
 *************************************************************************/
 ...
 * notation.
 *
 * When the number fits in the buffer it returns the buffer, otherwise
 * it returns NULL and sets extra to the extra space required.
 *************************************************************************/
char* log10_evalue_to_string(double log10_ev, int prec, char *buffer,
    int buffer_size, int *extra) {
  int size_req;
  double m, e;
  if (log10_ev > -HUGE_VAL && log10_ev < HUGE_VAL) { // normal value
    ...
    size_req = snprintf(buffer, buffer_size, "%.*fe%+04.0f", prec, m, e);
  } else if (log10_ev >= HUGE_VAL) { // infinity
    strncpy(buffer, "inf", buffer_size);
    size_req = 3;
  } else { // negative infinity
    size_req = snprintf(buffer, buffer_size, "%.*fe+000", prec, 0.0);
  }
  if (size_req < buffer_size) {
    if (extra) *extra = 0;
    return buffer;
  } else {
    if (extra) *extra = (size_req + 1) - buffer_size;
    return NULL;
  }
}
