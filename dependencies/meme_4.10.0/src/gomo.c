    ...
    )
{
  ...
  fprintf(xml_output,"\t<program name=\"%s\"\n"
      ...
      "\t\t\t min_gene_count=\"%d\"", 
      program_name, seed, command, gene_url, outdir, bool2txt(clobber), bool2txt(text_only), 
  ...
  ) 
{
  ...
  *imgs_path = NULL;
  ...
  char usage[] =
    ...
    "\n"
    "   Options:\n"
    "     --o <output dir>               Name of the directory for output. Will not replace an existing directory.\n"
    ...
    "     --shuffle_scores <n>           Generate empirical nulls by shuffling the sequence = score assignment. Repeat n times. (default=1000)\n"
    "     --score_E_thresh <double>      The E-value threshold above that all scored sequences obtain the same rank. (default=no threshold)\n"
    ...
    "     --seed <number>                Seed the random number generator. Different values will give slightly different outputs. (default = random)\n"
    ...
    "\n"
    "   See doc/gomo.html for information on the formats of <GO-term database> and <scoring file>.\n"
    "\n";
...
int main(int argc, char *argv[]) 
{
  ...
  if (!text_only) {
    ...
    //start output
    start_xml_output(xml_fp, seed, argc, argv, map_path, ama_paths, 
