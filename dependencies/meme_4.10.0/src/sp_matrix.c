...
) 
{
  ...
  if (init==0){
    ...
    MPI_Get_address(&seed_packet.num_seed_packets, &address[2]);
    MPI_Get_address(&seed_packet.seed, &address[3]);
    displacements[0]=0;
    ...
    MPI_Type_create_struct(4, block_lengths, displacements, typelist, &seed_packet_type);
