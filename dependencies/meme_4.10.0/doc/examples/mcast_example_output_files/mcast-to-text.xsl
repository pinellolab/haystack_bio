...
>
  ...
  <xsl:template match="/*">
    <xsl:text>#pattern name&#9;sequence name&#9;start&#9;stop&#9;score&#9;p-value&#9;E-value&#9;q-value&#9;matched sequence&#10;</xsl:text>
    <xsl:apply-templates select="cis:multi-pattern-scan/mem:match">
      <xsl:sort order="descending" data-type="number" select="../@p-value"/>
    </xsl:apply-templates>
  </xsl:template>
