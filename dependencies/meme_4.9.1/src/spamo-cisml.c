...
struct PRIMARY_LOADER {
  ...
  double calculated_score_threshold;
};
...
struct SECONDARY_LOADER {
  ...
  // the largest secondary motif Evalue considered significant
  double motif_evalue_cutoff;
  // the maximum distance from the primary that pvalues are calculated
  // only dump sequences of significant matches
  BOOLEAN_T dump_sig_only;
  int test_max;
  // the total number of secondary motifs
  ...
  int hit_count;
  // the minimum score allowed for a hit or (if negative) a multiplier for the maximum score
  double score_threshold_or_multiplier;
  // the background used to make PSSMs
  ARRAY_T *background;
  // the calculated score threshold
  double calculated_score_threshold;
};
...
void sequence_end_primary(void *ctx) {
  ...
  //check we're actually loading data
  if (loader->current_sequence == NULL) return;
  if (loader->hit_count == 0 || loader->current_score < loader->calculated_score_threshold) {
    match = 0;
 ...
 * for each sequence.
 **************************************************************************/
void load_spamo_primary(int margin, double score_threshold, ARRAY_T *background, 
    MOTIF_T *motif, RBTREE_T *sequences, const char *cisml) {
  ...
  if (score_threshold < 0 && score_threshold >= -1) {
    ...
    data.calculated_score_threshold = pssm_best_match_score(pssm) * (-score_threshold);
    free_pssm(pssm);
  } else {
    data.calculated_score_threshold = score_threshold;
  }
...
void sequence_end_secondary(void *ctx) {
  ...
  //check we're actually loading data
  if (loader->current_sequence == NULL) return;
  if (loader->hit_count == 0 || loader->secondary_score < loader->calculated_score_threshold) {
    match = 0;
  } else if (loader->hit_count == 1) {
    match = loader->hits[0];
  } else {
