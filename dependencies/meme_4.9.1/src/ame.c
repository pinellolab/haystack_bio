...
void rsc_getopt(int argc, char *argv[]) {
  const int num_options = 24; // change this if the number of options changes
...
const char* rsc_get_usage() {
  ...
  return
    ...
    "	  <sequence file> 	sorted FASTA sequences\n"
    ...
    "     --bgfile <background> (default: use frequencies in motif file)\n"
    "     --pvalue-report-threshold <pv> (default: 0.001)\n"
    "     --o  <output dir> (default=ame_out)\n"
    "     --oc <output dir> (default=ame_out)\n"
    "\n"
    "   Fisher test with either avg or max scoring (undefined with totalhits scoring) options:\n"
    "     --poslist       [fl|pwm] (default: fl)\n"
    "     --fl-threshold  <flt> (default: 0.001) (Requires --poslist fl)\n"
    "     --pwm-threshold <pwmt> (default: 1.0) (Requires --poslist pwm)\n"
    "     Hints: Be careful when switching the poslist. It switches between\n"
    ...
    "\n"
    "   Fisher, MHG, 4D-MHG, Ranksum tests with totalhits scoring options:\n"
    "     --length-correction (default: don't correct for length bias by subtracting #expected hits)\n"
    "     --pvalue-threshold <pvt> (default: 0.0002)\n"
    "\n"
    "   Ranksum-specific options:\n"
    "     --rsmethod [better|quick] (default: quick)\n"
    "\n"
    "   Linreg- and spearman-specific options:\n"
    "     --normalise-affinity (default: don't normalize motif scores so that the motifs are comparable)\n"
    "     --linreg-switchxy (default: don't switch the x-points (fluorescence scores) and the y-points (motif scores))\n"
    "     --log-fscores (default: regress on fluorescence scores (linreg) or ranks (spearman))\n"
    ...
    "\n"
    "   WARNING:\n"
    "     This tool will not re-sort input sequences. It assumes the FASTA file is sorted.\n"
    "     The FASTA ID field may be followed by a floating point value (e.g., a fluorescence score),\n"
    "     in which case the the file should be sorted in *increasing* order on these values.\n" 
    "     This value is only used in the 'linreg' mode.\n" 
    "\n"
    "   Citing AME:\n"
